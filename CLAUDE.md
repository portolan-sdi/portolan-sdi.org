...
- **Shared chrome:** the header and footer come from `SiteHeader` and `SiteFooter`. Do not inline `<header>` / `<footer>` markup in pages. The header is minimal — logo, the mono field-manual tagline (`nav.tagline` / `nav.taglineMeta`, right-aligned, hidden below `md`), and the locale control, on solid `--p-bg` with a black rule, no frosted `backdrop-blur`, no nav links, no hamburger. The footer is centered mono: a `( PORTOLAN )` text mark linking home, the site navigation row (Registry, Docs, GitHub), the tagline, and the governance/license/repo links in one `·`-separated row. The footer carries no colophon (removed July 2026 — don't reintroduce it) and only real links — never dead spans that look like links.
